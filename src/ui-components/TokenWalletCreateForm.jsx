...
export default function TokenWalletCreateForm(props) {
  ...
  const resetStateValues = () => {
    setTokenBalance(initialValues.tokenBalance);
    setName(initialValues.name);
    setTokenTransactions(initialValues.TokenTransactions);
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          const tokenWallet = await DataStore.save(new TokenWallet(modelFieldsToSave));
          const promises = [];
          promises.push(
            ...TokenTransactions.reduce((promises, original) => {
              promises.push(
                DataStore.save(
                  TokenTransaction.copyOf(original, (updated) => {
                    updated.tokenwalletID = tokenWallet.id;
                  })
                )
              );
              return promises;
      ...
      {...getOverrideProps(overrides, 'TokenWalletCreateForm')}
      {...rest}
    >
      <TextAreaField
        label="Token balance"
