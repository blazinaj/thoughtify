...
import { LessonCategory, Lesson, LessonLessonCategory } from '../models';
...
export default function LessonCategoryUpdateForm(props) {
  const {
    id: idProp,
    lessonCategory: lessonCategoryModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = lessonCategoryRecord
      ? { ...initialValues, ...lessonCategoryRecord, lessons: linkedLessons }
      : initialValues;
  ...
  const [lessonCategoryRecord, setLessonCategoryRecord] = React.useState(lessonCategoryModelProp);
  const [linkedLessons, setLinkedLessons] = React.useState([]);
  const canUnlinkLessons = false;
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp ? await DataStore.query(LessonCategory, idProp) : lessonCategoryModelProp;
      setLessonCategoryRecord(record);
    ...
    queryData();
  }, [idProp, lessonCategoryModelProp]);
  React.useEffect(resetStateValues, [lessonCategoryRecord, linkedLessons]);
  ...
  return (
    <Grid
        ...
        try {
          ...
          lessonsToUnLinkMap.forEach(async (count, id) => {
            ...
            const lessonLessonCategoryRecords = await DataStore.query(LessonLessonCategory, (r) =>
              r.and((r) => {
                return [r.lessonID.eq(recordKeys.id), r.lessonCategoryID.eq(lessonCategoryRecord.id)];
              })
            );
              ...
              promises.push(
                DataStore.save(
                  new LessonLessonCategory({
                    lessonCategory: lessonCategoryRecord,
                    lesson: lessonToLink
                  })
                )
              );
          ...
          promises.push(
            DataStore.save(
              LessonCategory.copyOf(lessonCategoryRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, 'LessonCategoryUpdateForm')}
      {...rest}
    >
      <TextField
        label="Name"
      ...
      <Flex justifyContent="space-between" {...getOverrideProps(overrides, 'CTAFlex')}>
        <Button
          ...
          isDisabled={!(idProp || lessonCategoryModelProp)}
        ...
        <Flex gap="15px" {...getOverrideProps(overrides, 'RightAlignCTASubFlex')}>
          <Button
            ...
            type="submit"
            variation="primary"
            isDisabled={!(idProp || lessonCategoryModelProp) || Object.values(errors).some((e) => e?.hasError)}
