...
export default function JournalEntryUpdateForm(props) {
  ...
  const resetStateValues = () => {
    const cleanValues = journalEntryRecord ? { ...initialValues, ...journalEntryRecord } : initialValues;
  ...
  const [journalEntryRecord, setJournalEntryRecord] = React.useState(journalEntryModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp ? await DataStore.query(JournalEntry, idProp) : journalEntryModelProp;
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            JournalEntry.copyOf(journalEntryRecord, (updated) => {
      ...
      <Flex justifyContent="space-between" {...getOverrideProps(overrides, 'CTAFlex')}>
        ...
        <Flex gap="15px" {...getOverrideProps(overrides, 'RightAlignCTASubFlex')}>
          <Button
            ...
            type="submit"
            variation="primary"
            isDisabled={!(idProp || journalEntryModelProp) || Object.values(errors).some((e) => e?.hasError)}
