...
export default function LessonUpdateForm(props) {
  ...
  const resetStateValues = () => {
    ...
    setLearningObjectives(
      cleanValues.learningObjectives?.map((item) => (typeof item === 'string' ? item : JSON.stringify(item))) ?? []
    );
    setCurrentLearningObjectivesValue('');
    setImages(cleanValues.images?.map((item) => (typeof item === 'string' ? item : JSON.stringify(item))) ?? []);
  ...
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp ? await DataStore.query(Lesson, idProp) : lessonModelProp;
      ...
      const linkedLessonNodes = record ? await record.LessonNodes.toArray() : [];
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          linkedLessonLabels.forEach((r) => {
            const count = linkedLessonLabelsMap.get(getIDValue.LessonLabels?.(r));
            const newCount = count ? count + 1 : 1;
          ...
          lessonLabelsToUnLinkMap.forEach(async (count, id) => {
            ...
            const lessonLessonLabelRecords = await DataStore.query(LessonLessonLabel, (r) =>
              r.and((r) => {
                return [r.lessonLabelID.eq(recordKeys.id), r.lessonID.eq(lessonRecord.id)];
              })
            );
          ...
          LessonCategories.forEach((r) => {
            const count = lessonCategoriesMap.get(getIDValue.LessonCategories?.(r));
            const newCount = count ? count + 1 : 1;
            lessonCategoriesMap.set(getIDValue.LessonCategories?.(r), newCount);
          });
          linkedLessonCategories.forEach((r) => {
            const count = linkedLessonCategoriesMap.get(getIDValue.LessonCategories?.(r));
            const newCount = count ? count + 1 : 1;
            linkedLessonCategoriesMap.set(getIDValue.LessonCategories?.(r), newCount);
          });
          ...
          lessonCategoriesToUnLinkMap.forEach(async (count, id) => {
            const recordKeys = JSON.parse(id);
            const lessonLessonCategoryRecords = await DataStore.query(LessonLessonCategory, (r) =>
              r.and((r) => {
                return [r.lessonCategoryID.eq(recordKeys.id), r.lessonID.eq(lessonRecord.id)];
              })
            );
            for (let i = 0; i < count; i++) {
          ...
          LessonNodes.forEach((r) => lessonNodesSet.add(getIDValue.LessonNodes?.(r)));
          linkedLessonNodes.forEach((r) => linkedLessonNodesSet.add(getIDValue.LessonNodes?.(r)));
      ...
      <Flex justifyContent="space-between" {...getOverrideProps(overrides, 'CTAFlex')}>
        ...
        <Flex gap="15px" {...getOverrideProps(overrides, 'RightAlignCTASubFlex')}>
          <Button
            ...
            type="submit"
            variation="primary"
            isDisabled={!(idProp || lessonModelProp) || Object.values(errors).some((e) => e?.hasError)}
