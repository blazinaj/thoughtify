...
export default function EnrollmentUpdateForm(props) {
  ...
  React.useEffect(() => {
    const queryData = async () => {
      ...
      setUser(UserRecord);
      const LessonRecord = record ? await record.Lesson : undefined;
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          TopicProgress.forEach((r) => {
            if (!linkedTopicProgressSet.has(getIDValue.TopicProgress?.(r))) {
              topicProgressToLink.push(r);
            }
          });
          topicProgressToUnLink.forEach((original) => {
            if (!canUnlinkTopicProgress) {
              throw Error(
                `EnrollmentTopicProgress ${original.id} cannot be unlinked from Enrollment because enrollmentID is a required field.`
              );
            }
            promises.push(
              DataStore.save(
          ...
          topicProgressToLink.forEach((original) => {
            promises.push(
              DataStore.save(
                EnrollmentTopicProgress.copyOf(original, (updated) => {
          ...
          promises.push(
            DataStore.save(
              Enrollment.copyOf(enrollmentRecord, (updated) => {
                ...
                if (!modelFieldsToSave.Lesson) {
                  updated.lessonID = undefined;
                }
                if (!modelFieldsToSave.Tutor) {
                  updated.enrollmentTutorId = undefined;
                }
