...
export default function PrintModelUpdateForm(props) {
  const {
    id: idProp,
    printModel: printModelModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = printModelRecord
      ? { ...initialValues, ...printModelRecord }
      : initialValues;
    setName(cleanValues.name);
    setDescription(cleanValues.description);
    setModelLink(cleanValues.modelLink);
    setModelNotes(cleanValues.modelNotes);
    setEstimatedPrintTime(cleanValues.estimatedPrintTime);
    setEstimatedVolume(cleanValues.estimatedVolume);
    setEstimatedCost(cleanValues.estimatedCost);
    setEstimatedWeight(cleanValues.estimatedWeight);
    setPrintSizeX(cleanValues.printSizeX);
    setPrintSizeY(cleanValues.printSizeY);
    setPrintSizeZ(cleanValues.printSizeZ);
    setFilamentType(cleanValues.filamentType);
    setErrors({});
  };
  const [printModelRecord, setPrintModelRecord] =
    React.useState(printModelModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(PrintModel, idProp)
        : printModelModelProp;
      setPrintModelRecord(record);
    };
    queryData();
  }, [idProp, printModelModelProp]);
  React.useEffect(resetStateValues, [printModelRecord]);
  const validations = {
    name: [{ type: "Required" }],
    description: [],
    modelLink: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            PrintModel.copyOf(printModelRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "PrintModelUpdateForm")}
      {...rest}
    >
      <TextField
        label="Name"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || printModelModelProp)}
