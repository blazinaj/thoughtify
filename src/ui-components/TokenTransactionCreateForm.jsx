...
export default function TokenTransactionCreateForm(props) {
  const {
    clearOnSuccess = true,
    onSuccess,
    onError,
  ...
  const [tokenwalletID, setTokenwalletID] = React.useState(
    initialValues.tokenwalletID
  );
  ...
  const resetStateValues = () => {
    ...
    setTokenwalletID(initialValues.tokenwalletID);
  ...
  const [
    currentTokenwalletIDDisplayValue,
    setCurrentTokenwalletIDDisplayValue,
  ] = React.useState("");
  const [currentTokenwalletIDValue, setCurrentTokenwalletIDValue] =
    React.useState(undefined);
  const tokenwalletIDRef = React.createRef();
  const [currentEnrollmentDisplayValue, setCurrentEnrollmentDisplayValue] =
    React.useState("");
  const [currentEnrollmentValue, setCurrentEnrollmentValue] =
    React.useState(undefined);
  ...
  const EnrollmentIdSet = new Set(
    Array.isArray(Enrollment)
      ? Enrollment.map((r) => getIDValue.Enrollment?.(r))
      : getIDValue.Enrollment?.(Enrollment)
  );
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(new TokenTransaction(modelFields));
          if (onSuccess) {
      ...
      {...getOverrideProps(overrides, "TokenTransactionCreateForm")}
      {...rest}
    >
      <TextField
        label="Timestamp"
      ...
      <ArrayField
        ...
        runValidationTasks={async () =>
          await runValidationTasks("tokenwalletID", currentTokenwalletIDValue)
        }
        errorMessage={errors?.tokenwalletID?.errorMessage}
        getBadgeText={(value) =>
          value
            ? getDisplayValue.tokenwalletID(
                tokenWalletRecords.find((r) => r.id === value)
              )
            : ""
        }
        setFieldValue={(value) => {
      ...
      >
        <Autocomplete
          ...
          onBlur={() =>
            runValidationTasks("tokenwalletID", currentTokenwalletIDValue)
          }
          errorMessage={errors.tokenwalletID?.errorMessage}
          hasError={errors.tokenwalletID?.hasError}
      ...
      <ArrayField
        ...
        runValidationTasks={async () =>
          await runValidationTasks("Enrollment", currentEnrollmentValue)
        }
        errorMessage={errors?.Enrollment?.errorMessage}
        getBadgeText={getDisplayValue.Enrollment}
        setFieldValue={(model) => {
          setCurrentEnrollmentDisplayValue(
            model ? getDisplayValue.Enrollment(model) : ""
          );
      ...
      >
        <Autocomplete
          ...
          onBlur={() =>
            runValidationTasks("Enrollment", currentEnrollmentDisplayValue)
          }
          errorMessage={errors.Enrollment?.errorMessage}
          hasError={errors.Enrollment?.hasError}
          ref={EnrollmentRef}
