...
export default function PrinterUpdateForm(props) {
  const {
    id: idProp,
    printer: printerModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = printerRecord
      ? { ...initialValues, ...printerRecord }
      : initialValues;
  ...
  const [printerRecord, setPrinterRecord] = React.useState(printerModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Printer, idProp)
        : printerModelProp;
      setPrinterRecord(record);
    };
    queryData();
  }, [idProp, printerModelProp]);
  React.useEffect(resetStateValues, [printerRecord]);
  const validations = {
    name: [{ type: "Required" }],
    model: [],
    serialNumber: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Printer.copyOf(printerRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "PrinterUpdateForm")}
      {...rest}
    >
      <TextField
        label="Name"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || printerModelProp)}
