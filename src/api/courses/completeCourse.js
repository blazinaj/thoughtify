...
export const completeCourse = async ({ course, user }) => {
  ...
  const lessons = await DataStore.query(Lesson, (lesson) =>
    lesson.and((lesson) => [lesson.courseID('eq', course.id), lesson.Enrollments.userID('eq', user.username)])
  );
  ...
  if (courseCompletion) {
    await DataStore.save(
      new Certification({
        ...
        description: `${user.firstName} ${user.lastName} has completed the course ${
          course.name
        } and has been awarded this certification on ${new Date().toLocaleDateString()}`,
        courseID: course.id,
