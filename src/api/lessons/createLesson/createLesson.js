...
export const createLesson = async (
  ...
  if (generateLessonCategories) {
    ...
    for (const category of categoryArray) {
      ...
      const foundCategory = lessonCategories?.find(existing => existing.name.toLowerCase() === category.toLowerCase());
      ...
      for (const topicResponse of parsedResult) {
        ...
        if (generateLessonTopicContent) {
          ...
          const topicPromise = generate({prompt}).then(async (result) => {
            ...
            const dataStorePromise = await DataStore.save(new LessonNode({
